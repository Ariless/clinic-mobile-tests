...
Before(async function () {
  if (process.env.PLATFORM !== 'ios') {
    // forceStop → clearAsyncStorage (run-as rm RKStorage) → openApp via monkey.
    // clearAsyncStorage removes only the JWT without pm clear, avoiding SystemUI ANR (MOB-C-85)
    // and UiAutomator2 context loss (MOB-C-86). App starts at the login screen every time.
    try { ADB.forceStop(APP_PACKAGE) } catch { /* best-effort */ }
    await browser.pause(1500)
    try { ADB.clearAsyncStorage(APP_PACKAGE) } catch { /* best-effort */ }
    try { ADB.openApp(APP_PACKAGE) } catch { /* best-effort */ }
    // Wait for login screen instead of fixed pause — handles Metro recompilation after code changes.
    await $('//*[@resource-id="login-title"]').waitForDisplayed({ timeout: 90000 }).catch(() => {})
  } else {
    try { await driver.terminateApp(APP_PACKAGE) } catch { /* best-effort */ }
